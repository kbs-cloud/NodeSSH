...
export const ProfileManager: React.FC = () => {
  ...
  const filteredProfiles = profiles.filter(p => {
    if (selectedFolder && (p.folder || 'Default') !== selectedFolder) return false;
  ...
  return (
    <div className="flex-1 h-full overflow-y-auto p-5 bg-[var(--theme-bg-dark,#070913)] space-y-6 select-none">
      ...
      {/* Quick Connect Bar Card */}
      <div className="p-4 rounded-xl bg-[#0e1222] border border-[var(--theme-border,#1e2640)] shadow-lg">
        <div className="text-xs font-semibold text-slate-300 mb-2 flex items-center gap-2">
          <Zap className="w-4 h-4 text-cyan-400" />
          <span>Quick Connect (Ad-Hoc Session)</span>
        </div>
        <QuickConnectBar />
      </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2 flex-1 max-w-md bg-[#0e1222] border border-[var(--theme-border,#1e2640)] rounded-lg px-3 py-1.5 text-xs">
          ...
          <input
            type="text"
            placeholder="Search profiles by name, host, user, or tag..."
            value={search}
          ...
          <div className="flex items-center gap-1 bg-[#0e1222] border border-[var(--theme-border,#1e2640)] rounded-lg p-1 text-xs">
            <button
              ...
              className={`px-2.5 py-1 rounded transition-colors ${
                selectedFolder === null ? 'bg-cyan-500/20 text-cyan-300 font-semibold' : 'text-slate-400 hover:text-white'
              }`}
            >
              All Folders ({profiles.length})
            </button>
            {folders.map(f => (
              <button
                key={f}
                ...
                className={`px-2.5 py-1 rounded transition-colors ${
                  selectedFolder === f ? 'bg-cyan-500/20 text-cyan-300 font-semibold' : 'text-slate-400 hover:text-white'
                }`}
              >
                {f}
              </button>
      ...
      {filteredProfiles.length === 0 ? (
        <div className="p-12 text-center border border-dashed border-white/10 rounded-xl bg-black/20 text-slate-500 text-xs">
          No server profiles found matching your search. Click &quot;New Server Profile&quot; to create one.
        </div>
      ) : (
